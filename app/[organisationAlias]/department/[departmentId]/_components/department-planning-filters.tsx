...
type DepartmentPlanningFiltersProps = {
  fromLocalISO: string;
  toLocalISO: string;
...
export function DepartmentPlanningFilters({
  ...
  organisationAlias,
  departmentId,
}: DepartmentPlanningFiltersProps) {
  ...
  useEffect(() => {
    if (dateRange?.from && dateRange?.to) {
      ...
      if (newFrom !== fromLocalISO || newTo !== toLocalISO) {
        const params = new URLSearchParams(searchParams.toString());
        params.set("from", newFrom);
        params.set("to", newTo);
        router.push(`/${organisationAlias}/department/${departmentId}?${params.toString()}`);
      }
    }
  }, [dateRange, fromLocalISO, toLocalISO, organisationAlias, departmentId, router, searchParams]);
